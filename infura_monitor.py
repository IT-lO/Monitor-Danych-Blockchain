        ...
        try:
            ...
            async with AsyncWeb3(WebSocketProvider(node_provider_endpoint)) as web3:
                if await web3.is_connected():
                    ...
                    await get_x_last_blocks(web3, 100)
            ...
            print(f"Problem with connection: {e}")
            await asyncio.sleep(5)


async def get_x_last_blocks(web3: AsyncWeb3, amount: int):
    output_dir = Path("filtered_blocks")
    output_dir.mkdir(exist_ok=True)
    ...
    for block in blocks:
        ...
        if isinstance(block, Exception):
            print(f"Error while retrieving a block: {block}")
            continue
